...
DR_EXPLAIN.dataManager = (function() {
	...
	var API = {
		...
		getErrorInLocalChromeSearch: function() {
			return data_search.DREXPLAIN_ERROR_LOCAL_SEARCH_IN_CHROME;
		},
...
function isEmpty(sToCheck) {
   ...
   return false;
		...
		{
			if (isLocalSearch() && $.browser.chrome )
				throw Error("LocalSearchNotSupportInChrome");
		...
		performSearch: function( s ) {
			...
			else
			{
				try	{
					...
					if (e.message == "LocalSearchNotSupportInChrome")
					{
						var $msg = $( '<div class="b-tree__searchResultText">' + this.dataManager.getErrorInLocalChromeSearch() + '</div>' );
